...
| T-52 | Load `PB2:jazz:…` on a device or player that has only lofi (D-026) | The loop loads and plays with lofi; status `no kit jazz, using lofi`; omitted modifier groups take lofi's defaults; encoding now gives `PB2:lofi:…`. |
