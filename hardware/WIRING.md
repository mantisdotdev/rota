...
MIDI, sync, the jam link, and kit samples from the card (io/). The button backlights are driven from the second NeoTrellis's LEDs now (D-099); production's LED chain (D-068) has no LED under skip, which lights when armed on the EVT, so DVT either adds one or shows skip's arming on screen only. The sample pads are silent on the device until io/ reads the WAVs; the synth pads play.
