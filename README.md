...
- [spec/](spec/) holds every behaviour as a plain-language scenario with an ID that tests name ([scenarios.md](spec/scenarios.md)), the normative share-code grammar with its golden codes ([share-format.md](spec/share-format.md)), the kits as data ([kits/lofi/kit.json](spec/kits/lofi/kit.json)), from which `tools/kit_builder.py` generates the header the engine compiles in, and reference renders of every screen view ([screens/](spec/screens/)).
...
In the simulator the keys 1–8 are the pads; S, W, K, Z, D, E and space are split, swap, skip, undo, dice, show and play; A, B, C and shift+D are the sections; the arrow keys and the mouse wheel turn the knobs (left and right pick which); - and = are the volume; P saves the screen to `out/screens/`. Hold E for the share view, hold Z and E together for settings, and E again to come back. The first run shows the tutorial; `out/sdcard/tutorial-done` remembers that it ran. The audition latency of every pad press is printed as it is measured.
